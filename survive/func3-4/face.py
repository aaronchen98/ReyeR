...
# tf.flags.DEFINE_string("model_path", "/root/CSC4001/checkpoint/twinGan", "Path containing a checkpoint.")
...
# tf.flags.DEFINE_string('input_image_path', '/root/CSC4001/data/test_face',
...
# tf.flags.DEFINE_string('output_image_path', '/root/CSC4001/data/test_face_output',
#                         'Path to output the output image (or directory).')

# tf.flags.mark_flags_as_required(['model_path', 'output_tensor_name'])

# input_image_path = '/root/CSC4001/data/test_face'
num_output = None  
output_image_path = '/root/CSC4001/data/test_face_output'
model_path = "/root/CSC4001/checkpoint/twinGan"
image_hw = 256
input_tensor_name = 'sources_ph' 
output_tensor_name = 'custom_generated_t_style_source:0'  
...
def face2face(input_image_path='/root/CSC4001/data/test_face'):
  output_image_path = '/root/CSC4001/results/face/' + input_image_path.split('/')[-1].split('.')[0] + '.jpg'
  inferer = ImageInferer()
  if input_image_path:
    outputs, image_paths = inferer.infer(input_image_path, return_image_paths=True)
  else:
    ...
    outputs, image_paths = inferer.infer(input_image_path, return_image_paths=True, num_output=num_output)

  if isinstance(outputs, list):
    util_io.touch_folder(output_image_path)
    for i, output in enumerate(outputs):
      util_io.imsave(os.path.join(output_image_path, os.path.basename(image_paths[i])), output)
  else:
